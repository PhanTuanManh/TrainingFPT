...
namespace TrainingFPT.Controllers
{
    public class TrainerController : Controller
    {
        ...
        public IActionResult Index(string SearchString, string Status)
        {
            ...
            var dataUser = new UserQuery().GetAllUsers(SearchString, Status, 3);
        ...
        public async Task<IActionResult> Add(UserDetail user, IFormFile AvatarFile)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    int idInsetCate = new UserQuery().InsertItemUser(3, user.UserName, user.Password, user.Email, user.Phone, user.Address, user.BirthDay, user.Gender, user.ExtraCode, avatarNameFile, user.Education, user.ProgramingLang, user.ToeicScore, user.Skills, user.IpClient, user.Status);
                    if (idInsetCate > 0)
                    {
